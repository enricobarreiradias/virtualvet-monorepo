...
const ReportViewerModal = dynamic(
  () => import("../../components/pdf/ReportViewerModal"),
  {
    ssr: false,
...
export default function HistoryPage() {
  ...
  // Inicializa o searchTerm com o valor da URL (se existir)
  const [searchTerm, setSearchTerm] = useState(
    searchParams.get("search") || "",
  );

  // Sincroniza o searchTerm se a URL mudar (Navegação vinda do Relatório)
  useEffect(() => {
  ...
  const loadHistory = useCallback(async () => {
    ...
    const urlChronology = searchParams.get("chronology") || "";
  ...
  const handleViewOdontogram = () => {
    if (selectedItem) {
      router.push(
        `/evaluate/${selectedItem.animalId}?source=history&mode=readonly`,
      );
    }
  };
  ...
  const activeChronology = searchParams.get("chronology");
  ...
  return (
    <Box sx={{ p: 4, width: "100%" }}>
      ...
      {activeChronology && (
        <Alert severity="info" sx={{ mb: 3 }} onClose={clearFilter}>
          Filtrando por cronologia:{" "}
          <strong>{activeChronology.toUpperCase()}</strong>.
        </Alert>
      )}

      {activePathology && (
        <Alert severity="info" sx={{ mb: 3 }} onClose={clearFilter}>
          Filtrando por patologia:{" "}
          <strong>{activePathology.toUpperCase()}</strong>. Mostrando apenas
          animais afetados.
        </Alert>
      )}
                          ...
                          >
                            ...
                            <Tooltip title="Opções de Visualização">
                              <IconButton
                                size="small"
                                color="primary"
                                onClick={() => handleOpenViewOptions(row)}
                              >
                                <Visibility />
                              </IconButton>
                            </Tooltip>
      ...
      <ReportViewerModal
        ...
        user={
          currentUser
            ? {
                name: currentUser.fullName || currentUser.email || "Usuário",
