...
export const EvaluationService = {
  getPending: (page = 1, limit = 20, search = "", farm = "", client = "") =>
    api.get(
      `/evaluations/pending?page=${page}&limit=${limit}&search=${search}&filterFarm=${farm}&filterClient=${client}`,
    ),
  ...
  getAllHistory: (
    ...
    api.get(
      `/evaluations/history?page=${page}&limit=${limit}&search=${search}&filterFarm=${farm}&filterClient=${client}&filterPathology=${pathology}&filterChronology=${chronology}`,
    ),
